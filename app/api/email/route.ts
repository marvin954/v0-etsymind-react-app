...
export async function POST(req: NextRequest) {
  ...
  if (!resendKey || !toEmail)
    return NextResponse.json({ error: "RESEND_API_KEY or REPORT_EMAIL not configured" }, { status: 500 });

  try {
    const { subject, analyticsData, createdProducts } = await req.json();
    ...
    const html = `
      <div style="font-family:sans-serif;max-width:600px;margin:0 auto;background:#f9f9f9;padding:32px;border-radius:12px">
        ...
        ${products ? `
        <div style="background:#fff;border-radius:8px;padding:20px;margin-bottom:16px">
          <h2 style="font-size:15px;color:#333;margin-bottom:12px">🎨 New Products Created Today</h2>
          <ul style="color:#555;padding-left:20px">${products}</ul>
        </div>` : ""}

        ${insights ? `
        <div style="background:#fff;border-radius:8px;padding:20px;margin-bottom:16px">
          <h2 style="font-size:15px;color:#333;margin-bottom:12px">💡 Key Insights</h2>
          <ul style="color:#555;padding-left:20px">${insights}</ul>
        </div>` : ""}

        ${actions ? `
        <div style="background:#fff;border-radius:8px;padding:20px;margin-bottom:16px">
          <h2 style="font-size:15px;color:#333;margin-bottom:12px">🚀 Recommended Actions</h2>
          <ul style="color:#555;padding-left:20px">${actions}</ul>
        </div>` : ""}

        <p style="color:#aaa;font-size:11px;text-align:center;margin-top:24px">
          Sent by EtsyMind AI • Autonomous Etsy Store Manager • Runs daily at 9am UTC
        </p>
      </div>`;
    ...
    const res = await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({
        from: "EtsyMind AI <onboarding@resend.dev>",
        to: toEmail,
        subject: subject || "EtsyMind AI Daily Report",
        html,
      }),
    });
